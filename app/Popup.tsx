...
const Popup = ({
  ...
  return (
    <Fragment>
      <div
        className={`z-50 fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[32rem] border-2  rounded-md overflow-hidden ${className}`}
      >
        <Button
      ...
      <div
        className="z-40 fixed top-0 left-0 w-full h-full  bg-opacity-50 backdrop-blur-sm"
        onClick={() => setIsOpen(false)}
      />
    </Fragment>
  );
