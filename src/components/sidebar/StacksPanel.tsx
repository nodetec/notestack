...
function StackItemDisplay({ stack, itemIndex, onSelectBlog, onDeleteItem, isDeleting }: StackItemDisplayProps) {
  ...
  if (isLoading) {
    return (
      <div className="px-4 py-2 text-sm text-zinc-400 flex items-center gap-2">
  ...
  if (!blog) {
    return (
      <div className="group relative px-4 py-2 text-sm text-zinc-400 italic flex items-center justify-between">
        <span>Article not found</span>
        <button
          onClick={() => onDeleteItem(stack, item)}
          disabled={isDeleting}
          className="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-red-100 dark:hover:bg-red-900/30 text-zinc-500 hover:text-red-600 dark:text-zinc-400 dark:hover:text-red-400 disabled:opacity-50 transition-opacity"
  ...
  return (
    <div className="group relative">
      <button
        ...
        className="w-full text-left px-4 py-2 pr-10 hover:bg-zinc-200 dark:hover:bg-zinc-800 transition-colors"
      >
      ...
      <button
        ...
        disabled={isDeleting}
        className="absolute right-2 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-red-100 dark:hover:bg-red-900/30 text-zinc-500 hover:text-red-600 dark:text-zinc-400 dark:hover:text-red-400 disabled:opacity-50 transition-opacity"
        title="Remove from stack"
        aria-label="Remove from stack"
      >
        {isDeleting ? (
...
function StackDisplay({ stack, onSelectBlog, onDeleteStack, onDeleteItem, isDeletingStack, deletingItemKey }: StackDisplayProps) {
  ...
  return (
    <Collapsible open={isOpen} onOpenChange={setIsOpen}>
      <div className="group relative">
        <CollapsibleTrigger className="flex w-full items-center justify-between px-3 py-2 hover:bg-zinc-200 dark:hover:bg-zinc-800 transition-colors">
          ...
          <ChevronDownIcon className={`w-4 h-4 text-zinc-400 transition-transform ${isOpen ? '' : '-rotate-90'}`} />
        </CollapsibleTrigger>
        <button
          onClick={(e) => {
            e.stopPropagation();
          ...
          className="absolute right-8 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-red-100 dark:hover:bg-red-900/30 text-zinc-500 hover:text-red-600 dark:text-zinc-400 dark:hover:text-red-400 disabled:opacity-50 transition-opacity"
          title="Delete stack"
      ...
      <CollapsibleContent>
        <div className="border-l-2 border-zinc-200 dark:border-zinc-700 ml-3">
          {stack.items.length === 0 ? (
            <p className="px-4 py-2 text-sm text-zinc-400 italic">
